...
router.get('/', async (req, res) => {
  res.render('shopping-cart', { title: 'Figaro - Registrar venda' });
});
...
router.get('/:query', async (req, res, next) => {
  try {
    ...
    let products: SKU[] = query ? await skuService.searchProductAsync(query) : [];
...
async function validateSale(obj: any): Promise<Sale> {
  ...
  if (items.some((item: SaleItem) => !Number(item.skuId) || !Number(item.quantity))) {
  ...
  sale.items = skus.map((sku) => Object.assign(new SaleItem(), {
    skuId: sku.id,
    name: sku.name,
  ...
  sale.totalPrice = sale.items.reduce((total, item) => (item.priceSold * item.quantity) + total, 0);
